{if $products}
	...
	{$th = $t1|default:0 + $t2|default:0 + $t3|default:0 + $t4|default:0 + $t5|default:0 + $t6|default:0 + $pd + 22}
                    ...
                    <div class="ty-column{$columns}"{if $ut2_load_more && $smarty.foreach.sprod.first && $smarty.foreach.sproducts.first} data-ut2-load-more="first-item"{/if}>
                        {if $product}
                            ...
                            <div class="ut2-gl__item {if $settings.abt__ut2.product_list.decolorate_out_of_stock_products == "Y" && $product.amount <= 0}out-of-stock{/if}" style="height: {$smarty.capture.abt__ut2_gl_item_height nofilter}px">
                                ...
                                <div class="ut2-gl__body" style="min-height: {if $settings.abt__ut2.product_list.$tmpl.grid_item_height[$settings.abt__device]}{if $settings.abt__ut2.product_list.$tmpl.show_sku[$settings.abt__device] == 'Y'}{$product_height|intval + 16}{else}{$product_height|intval+ 22}{/if}{else}{$th}{/if}px">
                                    ...
                                    {/if}
                                        ...
                                        {$b_feature=$product.abt__ut2_features[$settings.abt__ut2.general.brand_feature_id]}
										<div class="ypi-brand secblk">{$b_feature.variant}</div>
